...
export default function ValidatePage() {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!result) {
        result = await certificateRepo.findByVerificationCode(query.toUpperCase());
      }
      ...
      if (isCertificatePubliclyAvailable(result.status)) {
        ...
        setStatus('invalid');
        setMessage(
          `Certificado encontrado, pero su estado actual es "${getCertificateStatusLabel(result.status)}".`
        );
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-3xl shadow-sm p-8 border border-gray-100">
        <form onSubmit={handleVerify} className="space-y-6">
          <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700">Ingrese folio, codigo o UUID</label>
        ...
        {status === 'valid' && certificate && (
          <div className="mt-8 p-6 bg-green-50 rounded-2xl border border-green-100 flex items-start gap-4 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="space-y-2">
              ...
              <Link
                href={`/verify/${encodeURIComponent(certificate.publicVerificationCode || certificate.folio)}`}
            ...
            <div>
                ...
                {certificate && (
                  <p className="mt-2 text-sm text-red-800">
                    Estado actual: <strong>{getCertificateStatusLabel(certificate.status)}</strong>
                  </p>
                )}
              </div>
            </div>
